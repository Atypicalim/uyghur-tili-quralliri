...
var _$Tools_LA_$Class = function(base1,base2,ex) {
	this.ex = null;
	this.base2 = null;
	this.base1 = null;
	this.base1 = base1;
...
_$Tools_LA_$Class.__name__ = true;
_$Tools_LA_$Class.prototype = {
	__class__: _$Tools_LA_$Class
};
...
Reshaper.prototype = {
	...
	,ProLA_HAMZE: function(aWord) {
		...
		while(_g_current < _g_array.length) {
			...
			if(i == arr.length - 1) {
				...
				skipNext = false;
				continue;
			} else if(map.h[i] == Tools_la1.base2 && map.h[i + 1] == Tools_la1.base1) {
				result.push(Tools_la1.ex);
				skipNext = true;
			} else if(map.h[i] == Tools_la2.base2 && map.h[i + 1] == Tools_la2.base1) {
				result.push(Tools_la2.ex);
				skipNext = true;
			} else {
				result.push(map.h[i]);
			}
		...
		if(aWord.length < 2) {
			result.push(aWord);
		} else if(!(map.h[arr.length - 2] == Tools_la1.base1 && map.h[arr.length - 1] == Tools_la1.base2)) {
			if(!(map.h[arr.length - 2] == Tools_la2.base1 && map.h[arr.length - 1] == Tools_la2.base2)) {
	...
	,getExChar: function(character,alphabet) {
		var index = this.containsInArray(Tools_ALPHABET_BASE,character);
		if(index > -1) {
			if(alphabet == UyHarp.BASH) {
				return Tools_ALPHABET_EXT_BEGIN[index];
			} else if(alphabet == UyHarp.OTTURA) {
				return Tools_ALPHABET_EXT_MIDDLE[index];
			} else if(alphabet == UyHarp.AHIR) {
				return Tools_ALPHABET_EXT_END[index];
			} else if(alphabet == UyHarp.YALGHUZ) {
				return Tools_ALPHABET_EXT_SINGLE[index];
			}
		}
		return character;
	}
	,toExttended: function(text) {
		...
		var result = [];
		if(text.length == 1) {
			var index = this.containsInArray(Tools_ALPHABET_BASE,text);
			if(index == -1) {
				return "";
			} else {
				return Tools_ALPHABET_EXT_SINGLE[index];
		...
		if(text.length == 2) {
			var char1 = map.h[1];
			var char2 = map.h[2];
			if(this.containsInArray(Tools_ALPHABET_FRIENDLY,char1) > -1) {
		...
		while(_g_current < _g_array.length) {
			...
			var lastCharIndex = this.containsInArray(Tools_ALPHABET_BASE,lastChar);
			var currentIndex = this.containsInArray(Tools_ALPHABET_BASE,currentChar);
			var nextIndex = this.containsInArray(Tools_ALPHABET_BASE,nextChar);
			var currentCharExt = null;
			if(currentIndex == -1) {
				currentCharExt = currentChar;
			} else if(this.containsInArray(Tools_ALPHABET_FRIENDLY,lastChar) > -1) {
				if(nextIndex > -1) {
		...
		var solKolExt = null;
		if(this.containsInArray(Tools_ALPHABET_FRIENDLY,map.h[array.length - 2]) > -1) {
	...
	,toBasical: function(text) {
		...
		while(_g_current < _g_array.length) {
			...
			var base = map.h[i];
			var _g2_current = 0;
			var _g2_array = Tools_ALPHABET_BASE;
			while(_g2_current < _g2_array.length) {
				var _g3_value = _g2_array[_g2_current];
				var _g3_key = _g2_current++;
				var j = _g3_key;
				var w = _g3_value;
				var isA = Tools_ALPHABET_EXT_END[j] == map.h[i];
				var isB = Tools_ALPHABET_EXT_BEGIN[j] == map.h[i];
				var isO = Tools_ALPHABET_EXT_MIDDLE[j] == map.h[i];
				var isY = Tools_ALPHABET_EXT_SINGLE[j] == map.h[i];
				if(isA || isB || isO || isY) {
					base = Tools_ALPHABET_BASE[j];
					break;
				}
			}
			result.push(base);
...
var Tools = $hx_exports["Tools"] = function() { };
Tools.__name__ = true;
Tools.toExtended = function(text) {
	return Reshaper.toExt(text);
};
Tools.fromExtended = function(text) {
	return Reshaper.toBase(text);
};
Tools.toCommon = function(text) {
	return Converter.work("arabic","common",text);
};
Tools.toCyrillic = function(text) {
	return Converter.work("arabic","cyrillic",text);
};
Tools.toLatin = function(text) {
	return Converter.work("arabic","latin",text);
};
Tools.fromCommon = function(text) {
	return Converter.work("common","arabic",text);
};
Tools.fromCyrillic = function(text) {
	return Converter.work("cyrillic","arabic",text);
};
Tools.fromLatin = function(text) {
	return Converter.work("arabic","latin",text);
};
Tools.toAlphabet = function(text,isSHiftPressed) {
	return Keyboard.toAlphabet(text,isSHiftPressed);
};
Tools.fromAlphabet = function(alphabet,isSHiftPressed) {
	return Keyboard.fromAlphabet(alphabet,isSHiftPressed);
};
Tools.doSyllable = function(text) {
	return Syllable.parse(text);
};
Tools.readNumber = function(text) {
	return Numbers.read(text);
};
Tools.writeNumber = function(text) {
	return Numbers.write(text);
};
Tools.main = function() {
	haxe_Log.trace("\n\n--->TEST:",{ fileName : "Tools.hx", lineNumber : 1347, className : "Tools", methodName : "main"});
	haxe_Log.trace("converter:",{ fileName : "Tools.hx", lineNumber : 1350, className : "Tools", methodName : "main"});
	var origin = "ئالىمجان ۋە ئەسئەتجان سۇ تۇتتى.";
	haxe_Log.trace("origin",{ fileName : "Tools.hx", lineNumber : 1352, className : "Tools", methodName : "main", customParams : [origin]});
	var result = Tools.toCommon(origin);
	haxe_Log.trace("result",{ fileName : "Tools.hx", lineNumber : 1354, className : "Tools", methodName : "main", customParams : [result]});
	var reverse = Tools.fromCommon(result);
	haxe_Log.trace("reverse",{ fileName : "Tools.hx", lineNumber : 1356, className : "Tools", methodName : "main", customParams : [reverse]});
	haxe_Log.trace("alphabet:",{ fileName : "Tools.hx", lineNumber : 1359, className : "Tools", methodName : "main"});
	origin = "گ";
	haxe_Log.trace("origin",{ fileName : "Tools.hx", lineNumber : 1361, className : "Tools", methodName : "main", customParams : [origin]});
	result = Tools.toAlphabet(origin,true);
	haxe_Log.trace("result",{ fileName : "Tools.hx", lineNumber : 1363, className : "Tools", methodName : "main", customParams : [result]});
	reverse = Tools.fromAlphabet(result,true);
	haxe_Log.trace("reverse",{ fileName : "Tools.hx", lineNumber : 1365, className : "Tools", methodName : "main", customParams : [reverse]});
	haxe_Log.trace("numbers:",{ fileName : "Tools.hx", lineNumber : 1368, className : "Tools", methodName : "main"});
	origin = "123.40110001";
	haxe_Log.trace("origin",{ fileName : "Tools.hx", lineNumber : 1370, className : "Tools", methodName : "main", customParams : [origin]});
	result = Tools.readNumber(origin);
	haxe_Log.trace("result",{ fileName : "Tools.hx", lineNumber : 1372, className : "Tools", methodName : "main", customParams : [result]});
	reverse = Tools.writeNumber(result);
	haxe_Log.trace("reverse",{ fileName : "Tools.hx", lineNumber : 1374, className : "Tools", methodName : "main", customParams : [reverse]});
	haxe_Log.trace("reshaper:",{ fileName : "Tools.hx", lineNumber : 1377, className : "Tools", methodName : "main"});
	origin = "جەمئىيەت";
	haxe_Log.trace("origin",{ fileName : "Tools.hx", lineNumber : 1379, className : "Tools", methodName : "main", customParams : [origin]});
	result = Tools.toExtended(origin);
	haxe_Log.trace("result",{ fileName : "Tools.hx", lineNumber : 1381, className : "Tools", methodName : "main", customParams : [result]});
	reverse = Tools.fromExtended(result);
	haxe_Log.trace("reverse",{ fileName : "Tools.hx", lineNumber : 1383, className : "Tools", methodName : "main", customParams : [reverse]});
	haxe_Log.trace("syllable:",{ fileName : "Tools.hx", lineNumber : 1386, className : "Tools", methodName : "main"});
	origin = "ئاھلىرىم";
	haxe_Log.trace("origin",{ fileName : "Tools.hx", lineNumber : 1388, className : "Tools", methodName : "main", customParams : [origin]});
	result = Tools.doSyllable(origin);
	haxe_Log.trace("result",{ fileName : "Tools.hx", lineNumber : 1390, className : "Tools", methodName : "main", customParams : [result]});
	reverse = StringTools.replace(result," ","");
	haxe_Log.trace("reverse",{ fileName : "Tools.hx", lineNumber : 1392, className : "Tools", methodName : "main", customParams : [reverse]});
};
var Type = function() { };
Type.__name__ = true;
...
var Tools_KEYCODES = (function($this) {
	var $r;
...
var Tools_ALPHABET_BASE = ["ت","پ","ب","ر","د","چ","ج","ز","ف","ق","ك","ش","گ","س","ڭ","ن","م","ۋ","ل","خ","غ","ژ","ي","ا","ە","و","ۈ","ۆ","ۇ","ې","ى","ھ","ئ"];
var Tools_ALPHABET_EXT_BEGIN = ["ﺗ","ﭘ","ﺑ","ﺭ","ﺩ","ﭼ","ﺟ","ﺯ","ﻓ","ﻗ","ﻛ","ﺷ","ﮔ","ﺳ","ﯕ","ﻧ","ﻣ","ﯞ","ﻟ","ﺧ","ﻏ","ﮊ","ﻳ","ﺍ","ﻩ","ﻭ","ﯛ","ﯙ","ﯗ","ﯦ","ﯨ","ﮬ","ﺋ"];
var Tools_ALPHABET_EXT_SINGLE = ["ﺕ","ﭖ","ﺏ","ﺭ","ﺩ","ﭺ","ﺝ","ﺯ","ﻑ","ﻕ","ﻙ","ﺵ","ﮒ","ﺱ","ﯓ","ﻥ","ﻡ","ﯞ","ﻝ","ﺥ","ﻍ","ﮊ","ﻱ","ﺍ","ﻩ","ﻭ","ﯛ","ﯙ","ﯗ","ﯤ","ﻯ","ﮪ","ﺋ"];
var Tools_ALPHABET_EXT_MIDDLE = ["ﺘ","ﭙ","ﺒ","ﺮ","ﺪ","ﭽ","ﺠ","ﺰ","ﻔ","ﻘ","ﻜ","ﺸ","ﮕ","ﺴ","ﯖ","ﻨ","ﻤ","ﯟ","ﻠ","ﺨ","ﻐ","ﮋ","ﻴ","ﺎ","ﻪ","ﻮ","ﯜ","ﯚ","ﯘ","ﯧ","ﯩ","ﮭ","ﺌ"];
var Tools_ALPHABET_EXT_END = ["ﺖ","ﭗ","ﺐ","ﺮ","ﺪ","ﭻ","ﺞ","ﺰ","ﻒ","ﻖ","ﻚ","ﺶ","ﮓ","ﺲ","ﯔ","ﻦ","ﻢ","ﯟ","ﻞ","ﺦ","ﻎ","ﮋ","ﻲ","ﺎ","ﻪ","ﻮ","ﯜ","ﯚ","ﯘ","ﯥ","ﻰ","ﮫ","ﺌ"];
var Tools_ALPHABET_FRIENDLY = ["ت","ئ","خ","چ","ج","پ","ب","س","ش","غ","ف","ق","ك","گ","ڭ","ل","م","ن","ھ","ې","ى","ي"];
var Tools_REPLACE_MAP = (function($this) {
	var $r;
...
var Tools_la1 = new _$Tools_LA_$Class("ﻟ","ﺎ","ﻻ");
var Tools_la2 = new _$Tools_LA_$Class("ﻠ","ﺎ","ﻼ");
